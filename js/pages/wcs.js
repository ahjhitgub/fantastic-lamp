    ...
    async render(container) {
      ...
      const form = h(`
        <form class="panel">
          <h2>New WC</h2>
          <div class="field-row">
            <div class="field"><label>WC #</label><input name="wcNumber" required></div>
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          const id = await App.Store.createWC({ wcNumber: fd.get('wcNumber'), typeId: Number(fd.get('typeId')), date: fd.get('date') });
          App.Pages.wc.reset();
          App.UI.go(`#/wc/${id}`);
