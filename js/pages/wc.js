...
App.Pages.wc = (function () {
  ...
  function buildSaveBar(saved, data) {
    ...
    el.querySelector('[data-a="save"]').addEventListener('click', async () => {
      ...
      try {
        ...
        if (draft.kind === 'transfer') draft.companyId = draft.transfer.handlerId || draft.transfer.collectorId || null;
  ...
  function buildCommon(saved, data, bar) {
    ...
    const companyLabel = draft.kind === 'shipment' ? 'Destination' : 'Company';
    const el = h(`
      <div class="panel">
        <h2>Details</h2>
        <div class="field-row">
          <div class="field"><label>WC #</label><input data-f="wcNumber" value="${esc(draft.wcNumber)}"></div>
          <div class="field"><label>${isTransfer ? 'Date received' : 'Date'}</label><input type="date" data-f="date" value="${esc(draft.date)}"></div>
          ...
          ${isTransfer ? '' : `<div class="field"><label>${companyLabel}</label>
            <select data-f="companyId">${options(data.companies.map((c) => ({ value: c.id, label: c.name })), draft.companyId, '(none)')}</select></div>`}
  ...
  function buildParties(data, bar) {
    ...
    const el = h(`
      <div class="panel">
        <h2>Collector and handler</h2>
        <div class="field-row">
          <div class="field"><label>Approved collector (CEWID)</label><select data-f="collectorId">${pick('collector', t.collectorId)}</select></div>
    ...
    el.querySelectorAll('[data-f]').forEach((input) => input.addEventListener('input', () => {
      ...
      t[f] = f === 'activityNotes' ? input.value : (input.value ? Number(input.value) : null);
      bar.markDirty();
    }));
    return el;
  }
  ...
  function buildLines(bar) {
      ...
      return `
        <tr data-i="${i}">
          <td><select data-f="category">${catOptions(line.category)}</select>
              <input data-f="description" class="desc" placeholder="description (optional)" value="${esc(line.description)}"></td>
    ...
    function refresh() {
      ...
      problemsEl.replaceChildren(...math.problems.map((p) => U().notice(p, 'warning')));
      const wcRows = [];
      ...
      if (math.hasCrtOrPlasma) previewEl.append(U().notice(`197 note (added automatically): "${L().CRT_PLASMA_NOTE}"`, 'warning'));
    }
  ...
  function buildTimeline(bar) {
        ...
        <div class="timeline">
          ${steps.map(([key, label]) => `
            <div class="timeline-step" data-k="${key}">
              <label class="row"><input type="checkbox" data-role="done"> ${esc(label)}</label>
    ...
    function sync() {
      ...
      el.querySelectorAll('.timeline-step').forEach((row) => {
        ...
        row.querySelector('[data-role="done"]').disabled = na;
        const date = row.querySelector('[data-role="date"]');
        date.value = na ? '' : v; date.disabled = na;
  ...
  // ---------------------------------------------------------------- transfer: 197 preview
  function build197(saved, data, allocs) {
    const { h, esc, fmt } = U();
    const t = saved.transfer;
    const math = L().transferMath(t);
    const company = (id) => data.companies.find((c) => c.id === id);
    const coll = company(t.collectorId); const hand = company(t.handlerId);
    ...
    const activity = [hand ? `${hand.name} (Handler)` : null, math.hasCrtOrPlasma ? L().CRT_PLASMA_NOTE : null, t.activityNotes || null].filter(Boolean).join('. ');
    ...
    const claimedAll = months.reduce((s, k) => s + byMonth.get(k).NonCRT.units + byMonth.get(k).CBEP.units, 0);
    const needTables = months.length > 1 || (months.length === 1 && claimedAll !== total.units);
    const table = (title, k) => {
      const m = byMonth.get(k);
      return `<p><strong>${title}</strong> — Reporting Month/Year: ${k === undefined ? '' : L().monthLabelFromIndex(k)}</p>
        <table><thead><tr><th>CEW type</th><th class="num">Units</th><th class="num">Lbs</th></tr></thead><tbody>
          <tr><td>CA Sourced CRT CEW</td><td class="num">0</td><td class="num">0</td></tr>
          <tr><td>CA Sourced Non-CRT CEW</td><td class="num">${fmt(m ? m.NonCRT.units : 0)}</td><td class="num">${fmt(m ? m.NonCRT.weight : 0)}</td></tr>
          <tr><td>CA Sourced CBEP CEW</td><td class="num">${fmt(m ? m.CBEP.units : 0)}</td><td class="num">${fmt(m ? m.CBEP.weight : 0)}</td></tr>
        </tbody></table>`;
    };

    const el = h(`
      <div class="panel form-preview">
        <div class="row spread"><h2>CalRecycle 197 preview</h2><button type="button" data-a="print">Print</button></div>
        <p class="muted mt-0">Built from the saved WC. Signatures are done on paper.</p>
        <h3>I. Transfer Information</h3>
        <table><tbody>
          <tr><th>Date of transfer</th><td>${esc(saved.date)}</td><th>Approved collector</th><td>${esc(coll ? coll.name : '—')}</td><th>Collector CEWID #</th><td>${esc(coll ? coll.cewId : '')}</td></tr>
          <tr><th>Approved recycler</th><td colspan="3">${esc(data.profile.recyclerName || '— set in Settings —')}</td><th>Recycler CEWID #</th><td>${esc(data.profile.cewID || '')}</td></tr>
        </tbody></table>
        <h3>II. Transfer Amounts</h3>
        <table><thead><tr><th>CEW type</th><th class="num">Units transferred</th><th class="num">Weight (lbs)</th></tr></thead><tbody>
          <tr><td>CA Sourced CRT CEW</td><td class="num">${fmt(f.crt.units)}</td><td class="num">${fmt(f.crt.weight)}</td></tr>
          <tr><td>CA Sourced Non-CRT CEW</td><td class="num">${fmt(f.nonCrt.units)}</td><td class="num">${fmt(f.nonCrt.weight)}</td></tr>
          <tr><td>CA Sourced CBEP CEW</td><td class="num">${fmt(f.cbep.units)}</td><td class="num">${fmt(f.cbep.weight)}</td></tr>
          <tr><th>Totals</th><th class="num">${fmt(total.units)}</th><th class="num">${fmt(total.weight)}</th></tr>
        </tbody></table>
        <p><strong>Collector activity:</strong> ${esc(activity) || '<span class="muted">(none)</span>'}</p>
        <h3>V. Transfer Discrepancy Detail</h3>
        ${needTables ? table('Recycler Table 1', months[0]) + (months.length > 1 ? table('Recycler Table 2', months[1]) : '<p class="muted">Recycler Table 2: left blank until the rest is claimed.</p>')
          : '<p class="muted">Tables left blank — everything transferred is claimed in one month.</p>'}
      </div>`);
    el.querySelector('[data-a="print"]').addEventListener('click', () => {
      el.classList.add('print-area');
      window.addEventListener('afterprint', () => el.classList.remove('print-area'), { once: true });
      window.print();
    });
    return el;
  }
  ...
  return {
    async render(container) {
      ...
      if (saved.kind === 'transfer') {
        container.append(buildParties(data, bar), buildLines(bar), buildTimeline(bar), buildAllocations(saved, data, allocs), build197(saved, data, allocs));
