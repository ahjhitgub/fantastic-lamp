    ...
    async render(container) {
      ...
      const form = h(`
        <form class="panel">
          <h2>New transfer</h2>
          <div class="field-row">
            <div class="field"><label>WC # (= lot # on the cancellation log)</label><input name="wcNumber" required value="${esc(L.nextWcNumber(data.wcs))}"></div>
      ...
      form.addEventListener('submit', async (e) => {
        ...
        try {
          const id = await App.Store.createWC({ wcNumber: fd.get('wcNumber'), typeId: transferType.id, date: fd.get('date'), party: fd.get('party') });
          App.Pages.wc.reset();
          App.UI.go(`#/wc/${id}`);
      ...
      const rows = data.wcs.filter((w) => w.kind === 'transfer').map((w) => {
        ...
        const inv = L.invoiceMath({ transfer: w.transfer, mode: w.transfer.mode, priceItems: data.priceItems, company: P.customer });
        return { w, m, P, inv, allocs, status, done: App.Models.TRANSFER_TIMELINE.filter(([k]) => (k === 'wcAssigned' || k === 'materialReceived' ? w.date : tl[k])).length };
      });
      ...
      const list = rows.filter((r) => (!filters.status || r.status === filters.status)
        && (!q || [r.w.wcNumber, r.P.collector ? r.P.collector.name : '', r.P.handler ? r.P.handler.name : ''].some((s) => L.norm(s).includes(q))))
      ...
      container.append(h(`
        <div class="panel"><div class="table-scroll"><table>
          <thead><tr><th>WC #</th><th>Received</th><th>Handler</th><th>Collector</th><th>Material</th><th class="num">IRR</th><th class="num">Claimable CEW</th><th>Claimed in</th><th>Allocation</th><th class="num">Invoice</th><th>Timeline</th></tr></thead>
          <tbody>${list.map((r) => {
            ...
            return `<tr>
              ...
              <td>${esc(r.w.date)}</td>
              <td>${esc(r.P.handler ? r.P.handler.name : '—')}</td>
              <td>${esc(r.P.collector ? r.P.collector.name : '—')}${r.P.collectorIsFacility ? ' <span class="muted">(us)</span>' : ''}</td>
              <td>${r.w.transfer.mode === 'pickup' ? 'Picked up' : r.w.transfer.mode === 'dropoff' ? 'Dropped off' : '<span class="muted">—</span>'}</td>
              ...
              <td class="num">${r.inv.rows.length ? L.money(r.inv.total) : '—'}${r.inv.missing ? ` <span class="badge warn">${r.inv.missing} rate(s) needed</span>` : ''}</td>
