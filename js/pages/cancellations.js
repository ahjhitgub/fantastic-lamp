...
App.Pages.cancellations = (function () {
  ...
  async function transferOptions(periodId, selected) {
    const [transfers, collectors] = await Promise.all([
      getPeriodTransfers(periodId),
  ...
  async function transfersWeightForPeriod(periodId) {
    ...
    let weight = 0;
    transfers.forEach((t) => {
      weight += (Number(t.amounts?.nonCrt?.weight) || 0) + (Number(t.amounts?.cbep?.weight) || 0) + (Number(t.amounts?.crt?.weight) || 0);
    });
    return weight;
  }
    ...
    form.innerHTML = `
      ...
      <div class="field-row">
        ...
        <div class="field" style="flex:2;">
          ...
          <select name="originTransferId">${await transferOptions(periodId, u.originTransferId)}</select>
        </div>
      </div>
      <div class="row">
  ...
  async function renderUnitsTable(container, periodId, units) {
  ...
  return {
    async render(container) {
      ...
      if (viewMode === 'summary') {
        ...
        await renderUnitsTable(container, periodId, units);
      }
    },
  };
})();
