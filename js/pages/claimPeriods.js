...
App.Pages.claimPeriods = (function () {
  ...
  async function renderForm(container, period) {
    ...
    console.log('[DEBUG] renderForm() called, isEdit=', isEdit);
    const form = document.createElement('form');
    form.addEventListener('submit', (e) => e.preventDefault()); // safety net: never navigate, no matter what
    console.log('[DEBUG] safety-net listener attached to', form);
    ...
    console.log('[DEBUG] form appended, container now contains', container.querySelectorAll('form').length, 'form(s)');
    ...
    form.addEventListener('submit', async (e) => {
      console.log('[DEBUG] REAL submit handler fired for', form);
      e.preventDefault();
      ...
      try {
        if (isEdit) {
          ...
          console.log('[DEBUG] App.DB.add succeeded, new id=', id);
          App.State.currentPeriodId = id;
        }
        editingId = null;
        await App.refreshShell();
        console.log('[DEBUG] refreshShell finished after save');
      } catch (err) {
        console.log('[DEBUG] save threw:', err);
        showError(form, err);
      }
    });

    console.log('[DEBUG] real submit listener attached to', form);

    // Populate the dropdown last — purely cosmetic, doesn't need to block
    // the form being fully interactive.
    await refreshPrevOptions();
    console.log('[DEBUG] renderForm() fully finished for', form);
  }
  ...
  async function renderTable(container, periods) {
    ...
    panel.querySelectorAll('[data-delete]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        if (!confirm('Delete this claim period? Any transfer allocations into it will also be removed (the transfers themselves are not deleted).')) return;
        try {
  ...
  return {
    async render(container) {
      console.log('[DEBUG] claimPeriods.render() called with container:', container, 'at', new Date().toISOString());
      container.innerHTML = `
