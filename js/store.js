...
App.Store = (function () {
  ...
  async function loadAll() {
    const names = ['wcs', 'companies', 'claimPeriods', 'transferAllocations', 'wcTypes', 'wcStatuses', 'materials'];
    const [wcs, companies, periods, allocations, wcTypes, wcStatuses, materials] = await Promise.all(names.map((n) => App.DB.getAll(n)));
    ...
    return { wcs, companies, periods, allocations, wcTypes, wcStatuses, materials, profile };
  }
  ...
  async function createWC({ wcNumber, typeId, date, companyId }) {
    ...
    if (kind === 'transfer') {
      rec.transfer = { collectorId: null, handlerId: null, lines: [blankTransferLine()], timeline: { wcAssigned: App.UI.today() }, activityNotes: '' };
  ...
  return { loadAll, createWC, deleteWC, wcNumberTaken, ensurePeriod, findOrCreateCompany, addAliases, mergeCompanies, rewriteUnitCompanies, migrate, blankTransferLine, blankWeighLine };
})();
