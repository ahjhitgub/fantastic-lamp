...
const NAV_ITEMS = [
  ...
  { id: 'transfers', label: 'Transfers', group: 'Records' },
  { id: 'cancellations', label: 'Cancellations', group: 'Records' },
  { id: 'residuals', label: 'Residuals & Inventory', group: 'Records' },
  { id: 'disposition', label: 'Battery / Panel Disposition', group: 'Records' },
...
let __routeCallCount = 0;
async function route() {
  const callNum = ++__routeCallCount;
  console.log(`[DEBUG] route() call #${callNum}, hash=`, window.location.hash, 'at', new Date().toISOString());
  ...
  console.log(`[DEBUG] route() call #${callNum} finished`);
}
...
App.refreshShell = async function refreshShell() {
  console.log('[DEBUG] refreshShell() called at', new Date().toISOString());
